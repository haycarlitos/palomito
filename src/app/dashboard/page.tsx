...
export default function Dashboard() {
  ...
  // Fetch policies from smart contract
  const { data: contractPolicies, isLoading: isLoadingContract } = useReadContract({
  ...
  useEffect(() => {
    const fetchAndMergePolicies = async () => {
      ...
      try {
        ...
        // Convert contract policies to our format if available
        let contractPoliciesFormatted: Policy[] = [];
        if (contractPolicies && Array.isArray(contractPolicies)) {
          contractPoliciesFormatted = contractPolicies.map((cp: any) => {
            ...
            return {
              id: `onchain-${cp.id?.toString() || 'unknown'}`,
  ...
  }, [address, authenticated, ready, contractPolicies]);
